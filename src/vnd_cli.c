			 ...
			 struct net_buf_simple *buf)
{
	...
	LOG_DBG("Received NODE DETAILS STATUS message from 0x%04X,, data length %d", ctx->addr, buf->len);
			   ...
			   struct bt_mesh_vendor_status *rsp)
{
	...
	if (get) {	
		...
		net_buf_simple_add_u8(&msg, get->type); // Add the type byte if applicable
		...
		LOG_DBG("Sending GET message without type parameter");
		net_buf_simple_add_u8(&msg, BT_MESH_VENDOR_GET_TYPE_STATUS); // Default type
	}
